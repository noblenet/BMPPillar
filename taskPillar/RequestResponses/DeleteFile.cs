...
namespace PillarAPI.RequestResponses
{
    public class DeleteFile : IDeleteFile
    {
        ...
        public void ProcessRequest(IMessageInfoContainer message)
        {
            ...
            var receivedChecksumDataForFile = receivedDeleteFileRequestMessage.ChecksumDataForExistingFile;
            var responseInfo = new ResponseInfo();

            var collectionId = receivedDeleteFileRequestMessage.CollectionID;
            var fileName = receivedDeleteFileRequestMessage.FileID;
            var savedir =Pillar.GlobalPillarApiSettings.COLLECTION_FILE_DIRECTORY + receivedDeleteFileRequestMessage.CollectionID + @"\";
            ...
            if (fileInfoContainerForFileToDelete.FileStates != FileStatesEnum.ExistingInDB)
            {
                ...
                CollectedUtilities.InsertAudit(FileAction.DELETE_FILE.ToString(), receivedDeleteFileRequestMessage.From, receivedDeleteFileRequestMessage.AuditTrailInformation, receivedDeleteFileRequestMessage.FileID, responseInfo.ResponseText,Pillar.GlobalPillarApiSettings.PILLAR_ID);
                ReturnMessage(receivedDeleteFileRequestMessage, null, responseInfo);
                return;
            }

            var filePath = savedir + fileInfoContainerForFileToDelete.FileId + "." + fileInfoContainerForFileToDelete.FileSpecId + "." + fileName;
            // checks if file is on disk and either calculate or retrieve checksum, if right checksum is in DB.
            var checksumDataForFileTypeForFileToDelete = File.Exists(filePath) && !fileInfoContainerForFileToDelete.Archived ?
                                                                                  ChecksumUtilities.CalculateChecksumDataForFileType(receivedChecksumDataForFile.ChecksumSpec, filePath) :
                                                                                  CollectedUtilities.GetLatestChecksum(collectionId, fileName, receivedChecksumDataForFile.ChecksumSpec);
            ...
            if (checksumDataForFileTypeForFileToDelete != null && XmlUtilities.ValidateFileChkSum(checksumDataForFileTypeForFileToDelete.ChecksumValue, receivedChecksumDataForFile.ChecksumValue))
            {

                using (var myDb = DatabaseConnection.GetConnection())
                {
                    var retval = myDb.Update<FilePoco>("SET deleted = @deleted WHERE file_name = @file_name and user_id = (SELECT user_id FROM users WHERE collection_id = @collection_id)", 1, fileName, collectionId);
                    ...
                    try
                    {
                        if (retval >= 1)
                        {
                            ...
                            responseInfo.ResponseText = "Delete file request has been received and executed successfully";
                        }
                        else
                        {
                            responseInfo.ResponseCode = ResponseCode.FAILURE;
                            responseInfo.ResponseText = "Delete file request has been received. There is an error in the database, contact SA";
                        }
                    }
            ...
            else
            {
                ...
                {
                    responseInfo.ResponseText = "File deletion denied. File has been archived and no checksum for requested checksum type i available.";
                }
                else
                {
        ...
        private static void ReturnMessage(DeleteFileRequest deleteFileRequest, ChecksumDataForFile_TYPE responseChecksumDataForFile, ResponseInfo responseInfo)
        {
