    ...
    public class ChecksumGetChecksum : IGetChecksum
    {
        ...
        public void ProcessRequest(IMessageInfoContainer message)
        {
            ...
            try
            {
                ...
                if (!(IsGetAllChecksumRequest(receivedGetChecksumsRequest))) sqlstring.Append("AND f.file_name = @0", receivedGetChecksumsRequest.FileIDs.Item.ToString());

                using (var myDb = DatabaseConnection.GetConnection())
                {
                    var resChkTypeCollection = new List<object>();

                    var files = myDb.Query<string>(sqlstring);
                    foreach (var filename in files)
                    {
                        ChecksumDataForFile_TYPE checksumDataForFileTypeForFile = CollectedUtilities.GetLatestChecksum(receivedGetChecksumsRequest.CollectionID, filename, receivedGetChecksumsRequest.ChecksumRequestForExistingFile);
                    ...
                    if (files.Any())
                    {
                        ...
                        resInf.ResponseText = "Data has been found for get checksums request. If there are checksums missing contact SA and have them transfer files from disk";
                    }
                    else
                    {
        ...
        {
            return IsGetAllChecksumRequest(receivedGetChecksumsRequest) ? "AllFileIDs" : receivedGetChecksumsRequest.FileIDs.Item.ToString();
        }
