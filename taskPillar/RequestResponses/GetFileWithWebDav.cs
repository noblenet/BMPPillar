    ...
    public class GetFileWithWebDav : IGetFile
    {
        ...
        public void ProcessRequest(IMessageInfoContainer message)
        {
            ...
            if (receivedGetFileRequest != null)
            {
                ...
                {
                    Log.DebugFormat("Er i get og skal hente filen: {0}\\{1}.{2}.{3}", f1.FilePath, f1.FileId,
                                    f1.FileSpecId, fileName);
                    string filePath = string.Format("{0}.{1}.{2}", f1.FileId, f1.FileSpecId, fileName);
        ...
        {
            var _webdavSettings = new WebdavTransferSettings(Pillar.GlobalPillarApiSettings.WEBDAV_IP_ADDRESS,
            ...
            bool fileTransferResult = fileOperator.TransferFile(receivedGetFileRequest.FileAddress,
                                                                new FileInfo(filePath), receivedGetFileRequest.FileID);
            ResponseInfo responseInfo = SetResponseInfo(fileTransferResult);
