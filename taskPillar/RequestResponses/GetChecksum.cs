...
namespace PillarAPI.RequestResponses
{
    public class GetChecksum : IGetChecksum
    {
        ...
        public void ProcessRequest(IMessageInfoContainer message)
        {
            ...
            try
            {
                Database myDb = DatabaseConnection.GetConnection();
                myDb.OpenSharedConnection();

                IEnumerable<article> result = myDb.Query<article>(
                ...
                if (rowCount > 0)
                {
                    foreach (article Article in enumerable)
                    {
                        ChecksumDataForFile_TYPE checksumDataForFileTypeForFile =
                            CollectedUtilities.GetLatestChecksum(receivedGetChecksumsRequest.CollectionID,
                                                                 Article.file_name,
                        ...
                        if (Convert.ToBoolean(Article.archived))
                            // Archived -  if possible checksum will be fetched from DB.
                        {
                        ...
                        {
                            if (
                                File.Exists(filePath + Article.file_id + "." + Article.file_spec_id + "." +
                                            Article.file_name) && checksumDataForFileTypeForFile == null)
                            {
                                ChecksumDataForFile_TYPE checksumDataForFileTypeCalculated =
                                    ChecksumUtilities.CalculateChecksumDataForFileType(
                                        receivedGetChecksumsRequest.ChecksumRequestForExistingFile,
                                        filePath + Article.file_id + "." + Article.file_spec_id + "." +
                                        Article.file_name);
                                var resChkType = new ChecksumDataForChecksumSpec_TYPE
                                                     {
                                                         FileID = Article.file_name,
                                                         CalculationTimestamp =
                                                             checksumDataForFileTypeCalculated.CalculationTimestamp,
                                ...
                                var checksumDataForFileTypeForDb = new ChecksumDataForFile_TYPE
                                                                       {
                                                                           CalculationTimestamp =
                                                                               resChkType.CalculationTimestamp,
                    ...
                    resInf.ResponseText =
                        "Data for AllFileIDs has been found for get checksums request. If there are checksums missing contact SA and have them transfer files from disk";
                }
                else
                {
        ...
        private static void SingleFile(GetChecksumsRequest receivedGetChecksumsRequest)
        {
            ...
            string fullFilePath = Pillar.GlobalPillarApiSettings.COLLECTION_FILE_DIRECTORY + collectionId + @"\" +
                                  f1.FileId + "." + f1.FileSpecId + "." + fileName;
            ...
            {
                checksumDataForFileTypeForFile =
                    ChecksumUtilities.CalculateChecksumDataForFileType(
                        receivedGetChecksumsRequest.ChecksumRequestForExistingFile, fullFilePath);
                                             ...
                                             ResultingChecksums resChk)
        {
            ...
            CollectedUtilities.InsertAudit(FileAction.GET_CHECKSUMS.ToString(), receivedGetChecksumsRequest.From,
                                           receivedGetChecksumsRequest.AuditTrailInformation, filename,
