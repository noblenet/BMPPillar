...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
                    ...
                    using (IDbCommand cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        else
                        {
                            foreach (string colname in columns)
                            {
                                ...
                                if (index > 0)
                                    sb.Append(", ");
                                sb.AppendFormat("{0} = {1}{2}", _dbType.EscapeSqlIdentifier(colname), _paramPrefix,
                                                index++);
                        ...
                        cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2} = {3}{4}",
                                                        _dbType.EscapeTableName(tableName), sb,
        ...
        public int Update<T>(Sql sql)
        {
            PocoData pd = PocoData.ForType(typeof (T));
            return
                Execute(new Sql(string.Format("UPDATE {0}", _dbType.EscapeTableName(pd.TableInfo.TableName))).Append(sql));
        }
        ...
        public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            ...
            // Do it
            string sql = string.Format("DELETE FROM {0} WHERE {1}=@0", _dbType.EscapeTableName(tableName),
                                       _dbType.EscapeSqlIdentifier(primaryKeyName));
        ...
        {
            PocoData pd = PocoData.ForType(typeof (T));
            return Execute(string.Format("DELETE FROM {0} {1}", _dbType.EscapeTableName(pd.TableInfo.TableName), sql),
                           args);
        }
        ...
        public int Delete<T>(Sql sql)
        {
            ...
            return
                Execute(
                    new Sql(string.Format("DELETE FROM {0}", _dbType.EscapeTableName(pd.TableInfo.TableName))).Append(
                        sql));
        }
        ...
        public IEnumerable<TRet> Query<TRet>(Type[] types, object cb, string sql, params object[] args)
        {
            ...
            try
            {
                using (IDbCommand cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    Func<IDataReader, object, TRet> factory = MultiPocoFactory.GetFactory<TRet>(types,
                                                                                                _sharedConnection
                                                                                                    .ConnectionString,
                                                                                                sql, r);
                    if (cb == null)
	...
	*/
    ...
    public class ColumnInfo
    {
        ...
        public static ColumnInfo FromProperty(PropertyInfo pi)
        {
            ...
            bool ExplicitColumns =
                pi.DeclaringType.GetCustomAttributes(typeof (ExplicitColumnsAttribute), true).Length > 0;
    ...
    namespace Internal
    {
        ...
        internal class MultiPocoFactory
        {
            ...
            public static object GetAutoMapper(Type[] types)
            {
                ...
                return AutoMappers.Get(key, () =>
                                                {
                                                    // Create a method
                                                    var m = new DynamicMethod("petapoco_automapper", types[0], types,
                                                                              true);
                                                    ...
                                                    for (int i = 1; i < types.Length; i++)
                                                    {
                                                        ...
                                                        {
                                                            // Find the property
                                                            IEnumerable<PropertyInfo> candidates =
                                                            ...
                                                            il.Emit(OpCodes.Callvirt,
                                                                    candidates.First().GetSetMethod(true));
                                                            handled = true;
                                                        }

                                                        if (!handled)
                                                            throw new InvalidOperationException(
                                                                string.Format("Can't auto join {0}", types[i]));
                                                    }
                                                    ...
                                                    return
                                                        m.CreateDelegate(
                                                            Expression.GetFuncType(types.Concat(types.Take(1)).ToArray()));
                                                }
                    );
            }
                                                   ...
                                                   IDataReader r, ref int pos)
            {
                ...
                throw new InvalidOperationException(string.Format("Couldn't find split point between {0} and {1}",
                                                                  typeThis, typeNext));
            }
                                                                                        ...
                                                                                        string sql, IDataReader r)
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    Delegate del = FindSplitPoint(types[i], i + 1 < types.Length ? types[i + 1] : null, ConnectionString,
                                                  sql, r, ref pos);
                    dels.Add(del);
                ...
                return
                    ...
                    m.CreateDelegate(typeof (Func<IDataReader, object, TRet>), new MultiPocoFactory {_delegates = dels});
            }
            ...
            public static Func<IDataReader, object, TRet> GetFactory<TRet>(Type[] types, string ConnectionString,
                                                                           string sql, IDataReader r)
            {
                Tuple<Type, ArrayKey<Type>, string, string> key = Tuple.Create(typeof (TRet), new ArrayKey<Type>(types),
                                                                               ConnectionString, sql);

                return
                    (Func<IDataReader, object, TRet>)
                    MultiPocoFactories.Get(key,
                                           () =>
        ...
        internal class PocoData
        {
            ...
            private static readonly MethodInfo fnGetValue = typeof (IDataRecord).GetMethod("GetValue",
                                                                                           new[] {typeof (int)});
            ...
            {
                // Check cache
                Tuple<string, string, int, int> key = Tuple.Create(sql, connString, firstColumn, countColumns);
                ...
                return PocoFactories.Get(key, () =>
                                                  {
                                                      ...
                                                      var m =
                                                          new DynamicMethod(
                                                              "petapoco_factory_" + PocoFactories.Count.ToString(), type,
                                                      ...
                                                      if (type == typeof (object))
                                                      {
                                                          ...
                                                          for (int i = firstColumn; i < firstColumn + countColumns; i++)
                                                          {
                                                              ...
                                                              Func<object, object> converter =
                                                                  mapper.GetFromDbConverter(null, srcType);
							...
							*/
                                                              ...
                                                              // r[i]
                                                              il.Emit(OpCodes.Ldarg_0);
                                                                  // obj, obj, fieldname, converter?,    rdr
                                                              il.Emit(OpCodes.Ldc_I4, i);
                                                                  // obj, obj, fieldname, converter?,  rdr,i
                                                              il.Emit(OpCodes.Callvirt, fnGetValue);
                                                                  // obj, obj, fieldname, converter?,  value

                                                              // Convert DBNull to null
                                                              il.Emit(OpCodes.Dup);
                                                                  // obj, obj, fieldname, converter?,  value, value
                                                              il.Emit(OpCodes.Isinst, typeof (DBNull));
                                                                  // obj, obj, fieldname, converter?,  value, (value or null)
                                                              Label lblNotNull = il.DefineLabel();
                                                              il.Emit(OpCodes.Brfalse_S, lblNotNull);
                                                                  // obj, obj, fieldname, converter?,  value
                                                              ...
                                                              type == typeof (byte[]))
                                                          {
                                                              ...
                                                              Func<object, object> converter = GetConverter(mapper, null,
                                                                                                            srcType,
                                                                                                            type);
                                                          ...
                                                          else
                                                          {
                                                                   ...
                                                                   i++)
                                                              {
                                                                  ...
                                                                  // Do we need to install a converter?
                                                                  Func<object, object> converter = GetConverter(mapper,
                                                                                                                pc,
                                                                                                                srcType,
                                                                                                                dstType);
                                                                  ...
                                                                  if (converter == null)
                                                                  {
                                                                      ...
                                                                      if (valuegetter != null
                                                                          ...
                                                                          (valuegetter.ReturnType == dstType ||
                                                                           valuegetter.ReturnType ==
                                                                           Nullable.GetUnderlyingType(dstType)))
                                                                      {
                                                                          il.Emit(OpCodes.Ldarg_0); // *,rdr
                                                                          ...
                                                                          // Convert to Nullable
                                                                          if (Nullable.GetUnderlyingType(dstType) !=
                                                                              null)
                                                                          {
                                                                              il.Emit(OpCodes.Newobj,
                                                                                      dstType.GetConstructor(new[]
                                                                                                                 {
                                                                                                                     Nullable
                                                                                                                         .GetUnderlyingType
                                                                                                                         (dstType)
                                                              ...
                                                              MethodInfo fnOnLoaded = RecurseInheritedTypes(type,
                                                                                                            (x) =>
                                                                                                            x.GetMethod(
                                                                                                                "OnLoaded",
                                                                                                                BindingFlags
                                                      ...
                                                      // Cache it, return it
                                                      return
                                                          m.CreateDelegate(Expression.GetFuncType(typeof (IDataReader),
                                                                                                  type));
                                                  }
                    );
            }
            ...
            public static string AddSelectClause<T>(DatabaseType DatabaseType, string sql)
            {
                ...
                if (!rxSelect.IsMatch(sql))
                {
                    ...
                    string cols = pd.Columns.Count != 0
                                      ? string.Join(", ",
                                                    (from c in pd.QueryColumns
                                                     select tableName + "." + DatabaseType.EscapeSqlIdentifier(c))
                                                        .ToArray())
                                      : "NULL";
                ...
                return rxParams.Replace(sql, m =>
                                                 {
                                                     ...
                                                     {
                                                         var sb = new StringBuilder();
                                                         foreach (object i in arg_val as IEnumerable)
                                                         {
                                                             sb.Append((sb.Length == 0 ? "@" : ",@") +
                                                                       args_dest.Count.ToString());
                                                             args_dest.Add(i);
                                                         }
                                                         return sb.ToString();
                                                     }
